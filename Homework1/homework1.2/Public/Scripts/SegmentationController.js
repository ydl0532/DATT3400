...
function update(eventData) {
    ...
    // We might need to reconfigure tiles if aspect ratio of output camera changes.
    if (script.tiled) {
        configureTileImage();
    } 
}
...
function configureTileImage() {
    var deviceAspect = script.deviceCameraTexture.control.getAspect();
    
    if (lastKnownAspect === undefined || deviceAspect !== lastKnownAspect) {
        setAspectTileImage(deviceAspect);
        lastKnownAspect = deviceAspect;
    }
}

function setAspectTileImage(deviceAspect) {
    if (script.scrolling) {
    ...
    var imageAspect = billboardImage.control.getAspect();
